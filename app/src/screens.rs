...
impl Screens {
    ...
    pub fn build(ui: &mut Ui, screen_size: (f32, f32)) -> Result<Self> {
        ...
        panel.set_position(Anchor::TopLeft, menu_rect.x, menu_rect.y);
        panel.set_size(menu_rect.width, menu_rect.height);
        ...
        label.set_position(Anchor::TopLeft, 100.0, 100.0);

        let (resume_idx, btn) = ui.create_button(main_idx)?;
        btn.set_position(Anchor::TopLeft, 64.0, 200.0);
        btn.set_size(400.0, 48.0);
        ...
        label.set_position(Anchor::Left, 10.0, 0.0);

        let (game_btn_idx, btn) = ui.create_button(main_idx)?;
        btn.set_position(Anchor::TopLeft, 64.0, 296.0);
        btn.set_size(400.0, 48.0);
        btn.set_color(button_color);
        btn.set_hover_color(Some(button_hover_color));
        ...
        btn.set_position(Anchor::TopLeft, 64.0, 392.0);
        btn.set_size(400.0, 48.0);
        btn.set_color(button_color);
        btn.set_hover_color(Some(button_hover_color));
        ...
        btn.set_position(Anchor::TopLeft, 64.0, 488.0);
        btn.set_size(400.0, 48.0);
        btn.set_color(button_color);
        btn.set_hover_color(Some(button_hover_color));
        ...
        btn.set_position(Anchor::TopLeft, 64.0, 584.0);
        btn.set_size(400.0, 48.0);
        btn.set_color(button_color);
        btn.set_hover_color(Some(button_hover_color));
        ...
        btn.set_position(Anchor::TopLeft, 64.0, 680.0);
        btn.set_size(400.0, 48.0);
        btn.set_color(button_color);
        btn.set_hover_color(Some(button_hover_color));
        ...
        panel.set_position(Anchor::TopLeft, 64.0, 200.0);
        panel.set_size(400.0, 48.0);
        panel.set_color(row_color);
        let (_, label) = ui.create_label(row_idx)?;
        label.set_text("V-Sync");
        label.set_position(Anchor::Left, 8.0, 0.0);

        let initial = pending.get();
        let (vsync_checkbox_idx, checkbox) = ui.create_checkbox(row_idx)?;
        checkbox.set_position(Anchor::Right, -8.0, 0.0);
        checkbox.set_size(32.0, 32.0);
        ...
        panel.set_position(Anchor::TopLeft, 64.0, 296.0);
        panel.set_size(400.0, 48.0);
        panel.set_color(row_color);
        ...
        slider.set_position(Anchor::Right, -8.0, 0.0);
        ...
        label.set_width(fps_label_width);
        label.set_position_anchored_to(Anchor::Right, fps_slider_idx, Anchor::Left, -8.0, 0.0);
        ...
        frame.set_position(Anchor::TopLeft, 64.0, 264.0);
        ...
        for i in 0..row_count {
            ...
            label.set_position(Anchor::TopLeft, 8.0, row_offset / 2.0 + i as f32 * row_pitch);
        }

        // ── UI Tests ─────────────────────────────────────────────────────────
        let (ui_test_idx, panel) = ui.create_panel(0)?;
        panel.set_size(screen_size.0, screen_size.1);
        panel.set_color(panel_color);
        panel.set_visible(false);
        ...
        window.set_position(Anchor::TopLeft, panel_w + 40.0, 80.0);
        ...
        btn.set_position(Anchor::TopLeft, 10.0, 10.0);
        btn.set_size(100.0, 32.0);
        ...
        nested.set_position(Anchor::TopLeft, 150.0, 80.0);
        ...
        window.set_position(Anchor::TopLeft, panel_w + 140.0, 160.0);
        window.set_draggable(true);
        ...
        clamped.set_position(Anchor::TopLeft, 20.0, 30.0);
        ...
        win.set_position(Anchor::TopLeft, panel_w + 40.0, 360.0);
        ...
        pb_win.set_position(Anchor::TopLeft, panel_w + 300.0, 80.0);
        ...
        pb.set_position(Anchor::TopLeft, 8.0, 54.0);
        ...
        vt_lbl.set_position(Anchor::TopLeft, 8.0, 12.0);

        let (pb_checkbox_idx, checkbox) = ui.create_checkbox(pb_body_idx)?;
        checkbox.set_position(Anchor::TopLeft, 8.0 + vert_label_w + 8.0, 6.0);
        checkbox.set_size(20.0, 20.0);
        checkbox.set_hover_color(Some(button_hover_color));
        ...
        rev_lbl.set_position(Anchor::TopLeft, 8.0, 34.0);

        let (pb_rev_checkbox_idx, checkbox) = ui.create_checkbox(pb_body_idx)?;
        checkbox.set_position(Anchor::TopLeft, 8.0 + rev_label_w + 8.0, 28.0);
        checkbox.set_size(20.0, 20.0);
        checkbox.set_hover_color(Some(button_hover_color));
        ...
        pct_lbl.set_text("50%");
        pct_lbl.set_width(pct_w);
        pct_lbl.set_position(Anchor::TopLeft, 200.0, 188.0);

        let (pb_slider_idx, slider) = ui.create_slider(pb_body_idx, Axis::Horizontal)?;
        slider.set_position(Anchor::TopLeft, 8.0, 186.0);
        slider.set_size(186.0, 16.0);
        ...
        ui.get_node_mut::<CheckboxNode>(pb_checkbox_idx)?.interaction.on_release = Some(Box::new(move |ui: &mut Ui| {
            ...
            if vertical {
                let _ = ui.get_node_mut::<ProgressBarNode>(pb_idx).map(|pb| pb.set_size(14.0, 120.0));
                let _ = ui.set_axis::<ProgressBarNode>(pb_idx, axis);
                let _ = ui.get_node_mut::<SliderNode>(pb_slider_idx).map(|s| {
                    s.set_position(Anchor::TopLeft, 30.0, 54.0);
                    s.set_size(16.0, 120.0);
                    s.reversed = true;
                });
                if let Some(idx) = thumb_idx {
                    let _ = ui.get_node_mut::<ButtonNode>(idx).map(|b| b.set_size(32.0, 16.0));
                }
                let _ = ui.set_axis::<SliderNode>(pb_slider_idx, axis);
            } else {
                let _ = ui.get_node_mut::<ProgressBarNode>(pb_idx).map(|pb| pb.set_size(236.0, 14.0));
                let _ = ui.set_axis::<ProgressBarNode>(pb_idx, axis);
                let _ = ui.get_node_mut::<SliderNode>(pb_slider_idx).map(|s| {
                    s.set_position(Anchor::TopLeft, 8.0, 186.0);
                    s.set_size(186.0, 16.0);
                    s.reversed = false;
                });
                if let Some(idx) = thumb_idx {
                    let _ = ui.get_node_mut::<ButtonNode>(idx).map(|b| b.set_size(16.0, 32.0));
                }
                let _ = ui.set_axis::<SliderNode>(pb_slider_idx, axis);
            }
        }));
        ...
        world.set_size(screen_size.0, screen_size.1);
        ...
        hotbar.set_position(Anchor::Bottom, 0.0, -SLOT_MARGIN_BOTTOM);
        hotbar.set_size(total_w, SLOT_SIZE + SLOT_GAP);
        hotbar.set_color(panel_color);

        for i in 0..HOTBAR_SLOTS {
            let x = i as f32 * (SLOT_SIZE + SLOT_GAP) + SLOT_GAP;
            let (_, slot) = ui.create_button(hotbar_idx)?;
            slot.set_position(Anchor::TopLeft, x, SLOT_GAP / 2.0);
            slot.set_size(SLOT_SIZE, SLOT_SIZE);
        ...
        h_bar.set_position(Anchor::Center, 0.0, 0.0);
        h_bar.set_size(XH_SIZE * 2.0, XH_THICKNESS * 2.0);
        h_bar.set_color(xh_color);

        let (_, v_bar) = ui.create_panel(world_idx)?;
        v_bar.set_position(Anchor::Center, 0.0, 0.0);
        v_bar.set_size(XH_THICKNESS * 2.0, XH_SIZE * 2.0);
        ...
        world_node.set_on_show(|ui: &mut Ui| ui.request_cursor(CursorRequest::Lock));
        world_node.set_on_hide(move |ui: &mut Ui| {
            ui.request_cursor(CursorRequest::Free { x: mouse_center.0, y: mouse_center.1 });
        });

        // ── Debug overlay ────────────────────────────────────────────────────
        let (debug_idx, debug) = ui.create_group(0)?;
        debug.set_size(screen_size.0, screen_size.1);
        ...
        label.set_position(Anchor::Top, 0.0, 80.0);

        let (start_idx, start_btn) = ui.create_button(title_idx)?;
        start_btn.set_position(Anchor::Center, 0.0, 0.0);
        start_btn.set_size(200.0, 48.0);
        ...
        label.set_position(Anchor::Left, 64.0, 0.0);

        let (quit_idx, quit_btn) = ui.create_button(title_idx)?;
        quit_btn.set_position(Anchor::Center, 0.0, 64.0);
        quit_btn.set_size(200.0, 48.0);
